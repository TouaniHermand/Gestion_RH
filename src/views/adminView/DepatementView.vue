...
const index = async () =>{
    try {
       const result = await axios.get('http://127.0.0.1:3333/Departsemployes',{
        headers: {
           Authorization: 'Bearer ' + localStorage.getItem('token'),
        },
       })
...
const Create = async () =>{
    try {
        const result = await axios.post('http://127.0.0.1:3333/departements', {
            nom: nom.value,
        }, {
            headers: {
                Authorization: 'Bearer ' + localStorage.getItem('token'),
...
const toggleFormUpdate = async(id)=>{
    ...
    try {
       const result = await axios.get(`http://127.0.0.1:3333/departements/${id}`,{
        headers: {
           Authorization: 'Bearer ' + localStorage.getItem('token'),
        },
       })
       departement.value = result.data
       console.log(departement.value)

       nom_update.value = departement.value.nom;
    } catch(error){
        console.log(error.response.data);
    }
} 
...
const deleteDepartement = async (id)=>{
    try {
       const result = await axios.delete(`http://127.0.0.1:3333/departements/${id}`,{
        headers: {
           Authorization: 'Bearer ' + localStorage.getItem('token'),
        },
       })
...
const updateDepartement = async (id) => {
    try {
        const result = await axios.put(`http://127.0.0.1:3333/departements/${id}`, {
    ...
    <div class="main" :class="{ 'active': isActive }">
                ...
                <div class="details">
                            ...
                            <tbody>
                                <tr v-for="departement in departements" :key="departement.id">
                                  <td>{{ departement.nom }}</td>
                                    <td>
                                        <ul>
                                            <li v-for="employe in departement.employes" :key="employe.id">
                                                {{ employe.nom }} {{ employe.prenom }}
                                            </li>
                                        </ul>
                                    </td>
                                    <td>
                                        <ion-icon name="create" @click="toggleFormUpdate(departement.id)"></ion-icon>
                                        <ion-icon name="trash-sharp" @click="deleteDepartement(departement.id)"></ion-icon>
                                    </td>
                                </tr>
                            </tbody>
                        </table>
                    ...
                    <div class="recentCustumers" v-if="showFormUpdate">
                        ...
                        <form @submit.prevent ="updateDepartement(departement.id)" >     
                            <div class="form-floating mb-3 inputBox">
...
.details .recentOrders table tbody tr ul {
